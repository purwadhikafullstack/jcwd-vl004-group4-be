...
const DFGetCartItems = async (req, res) => {
    ...
    res.status(200).send({ rows, count, cartCount, totalPrice: cartPrice() })
}
...
const DFAddcart = async (req, res) => {
    ...
    const existingCart = await Cart.findOne({ where: { userId: req.body.userId, productId: req.body.productId } })

    if (existingCart) {
        if (existingCart.qty + req.body.qty > productData.stock) {
            res.send({ conflict: `Cannot add more item in your cart as you already have ${existingCart.qty}`, warning: `${productData.name} stock : ${productData.stock}` })
        } else {
            existingCart.qty = existingCart.qty + req.body.qty
            await existingCart.save()
            res.status(200).send({ message: `${productData.name} quantity  has been increased` })
        }
    } else {

        if (req.body.qty > productData.stock) {
            res.send({ conflict: `Input quantity exceeds stock! ${productData.name} stock: ${productData.stock}` })
        } else {
            const cart = await Cart.create(data)
            const count = await Cart.count({ where: { userId: req.body.userId } })
            res.status(200).send({ message: 'One item has been added to your cart', cart: cart, count })
        }
    }
}
...
const DFUpdate = async (req, res) => {
    ...
    const update = await Cart.update({ qty: req.body.qty }, { where: { id: id } })
    const { rows, count } = await Cart.findAndCountAll({ where: { userId: userId }, include: Product })
